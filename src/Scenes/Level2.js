class Level2 extends Phaser.Scene {
    ...
    create() {
        ...
        my.sprite.snake = this.add.follower(this.curve, 10, 10, "snake").setScale(0.25);
        this.createPath(my.sprite.snake, this.curve, this.points);
        my.sprite.snake.destroy();
        ...
        my.sprite.snake1 = this.add.sprite(500, 40, "snake").setScale(0.25);
        my.sprite.wing = this.add.sprite(300, 40, "wingMan1").setScale(0.25);
    ...
    update() { 
        ...
        // make enemies move towards player, be hit by the bullet, and collide with player 3 times
        // elephant
        ...
        if (this.enemyHitCounter2 < 3) {
            this.hitEnemy1(my.sprite.hippo1, my.sprite.bullet, 1);
        ...
        // snake
        this.enemyShootBullet(my.sprite.snake1, my.sprite.enemyBullet);
        if (this.enemyHitCounter3 < 3) {
            this.hitEnemy2(my.sprite.snake1, my.sprite.bullet, 3);
            if (my.sprite.snake1.visible = true) {
                this.moveToPlayer(my.sprite.snake1, my.sprite.penguin);
            }
            this.collideWithPLayer(my.sprite.snake1, my.sprite.penguin);
        }
        // wing creature
        if (this.enemyHitCounter4 < 3) {
            this.hitEnemy3(my.sprite.wing, my.sprite.bullet, 2);
            if (my.sprite.wing.visible = true) {
                this.moveToPlayer(my.sprite.wing, my.sprite.penguin);
            }
            this.collideWithPLayer(my.sprite.wing, my.sprite.penguin);
        }

        // if the enemies have been defeated, go to the next scene
        if (this.enemy1Destroyed && this.enemy2Destroyed && this.enemy3Destroyed && this.enemy4Destroyed) {
            this.enemyHitCounter = 0;
            this.enemyHitCounter2 = 0;
            this.enemyHitCounter3 = 0;
            this.enemyHitCounter4 = 0;
    ...
    moveAndCollideWithPlayer(enemy, num) {
        if (!enemy.active) return;
    
        this.hitEnemy(enemy, my.sprite.bullet, num);
        this.moveToPlayer(enemy, my.sprite.penguin);
        this.collideWithPLayer(enemy, my.sprite.penguin);
    }
    ...
    hitEnemy3(enemy, b, num) {
        for (let bullet of b) {
            if (this.collides(enemy, bullet)) {
                ...
                // Increment hit counter
                this.enemyHitCounter4++;
                // Check if enemy has been hit 3 times
                if (this.enemyHitCounter4 >= 3) {
                    enemy.visible = false;
                    enemy.destroy();
                    this.enemy4Destroyed = true;
                    return;
                }
